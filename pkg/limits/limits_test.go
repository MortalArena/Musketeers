...
func TestRateLimiter_TryAcquire(t *testing.T) {
	limiter := NewRateLimiter(5, 5, 1*time.Second) // maxTokens=5, refillRate=5
	...
	if limiter.Tokens() != 0 {
		t.Errorf("Expected 0 tokens remaining, got %d", limiter.Tokens())
	}
...
func TestConnectionLimiter_CleanupIdle(t *testing.T) {
	...
	// Wait a bit to make conn1 older
	time.Sleep(20 * time.Millisecond)

	// Add second connection
	limiter.Acquire("conn2")

	// Update activity for conn2 to keep it active
	limiter.UpdateActivity("conn2")

	// Wait a bit more
	time.Sleep(10 * time.Millisecond)

	// Cleanup idle connections (older than 20ms)
	count := limiter.CleanupIdle(20 * time.Millisecond)
